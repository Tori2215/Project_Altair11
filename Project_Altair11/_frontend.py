...
DEFAULT_CATEGORIES = {"Еда": 0.25, "Транспорт": 0.25, "Досуг": 0.25, "Сбережения": 0.25}
DEFAULT_WALLET = {"budget": 0.0, "expense_items": []}
GOAL_EXPENSE_CATEGORY = "Сбережения"
_PYARROW_AVAILABLE = None
_PYARROW_ERROR = None

# 3:55 — Исправлено: переводы в цели снова синхронизируются с расходами,
# главная страница показывает корректные суммы, загрузка JSON стала устойчивее.
# Улучшено: безопасный вывод пользовательских названий в HTML и более мягкая
# работа приложения при битых или неполных файлах данных.


def can_use_streamlit_dataframe():
    """Проверяет, доступен ли pyarrow для st.dataframe в текущем окружении."""
    global _PYARROW_AVAILABLE, _PYARROW_ERROR
    ...
    try:
        ...
        _PYARROW_ERROR = str(exc)

    return _PYARROW_AVAILABLE


def has_streamlit_context():
    """Проверяет, запущен ли код внутри streamlit run."""
    try:
...
def render_table(data, *, width="stretch", height=400):
    """Безопасно отображает таблицу, даже если pyarrow недоступен."""
    if can_use_streamlit_dataframe():
        st.dataframe(data, width=width, height=height)
        return

    fallback_df = pd.DataFrame(data)
    wrapper_style = "overflow-x: auto; margin: 0.5rem 0 0.75rem 0;"
    if height:
    ...
    st.markdown(
        ...
        <div style="{wrapper_style}">
            {fallback_df.to_html(index=False, classes="fallback-table", escape=True)}
        </div>
        """,
        unsafe_allow_html=True,
    )
    if has_streamlit_context() and not st.session_state.get("_pyarrow_fallback_notice_shown", False):
        st.caption(f"Интерактивная таблица недоступна: {_PYARROW_ERROR}")
        st.session_state["_pyarrow_fallback_notice_shown"] = True


def _as_float(value, default=0.0):
    """Преобразует значение к float и возвращает default при ошибке."""
    try:
        return float(value)
...
def _load_json_file(file_path, default_data):
    """Загружает JSON и возвращает копию default_data, если файл битый или отсутствует."""
    try:
...
def load_weights():
    ...
    """
    raw_weights = _load_json_file(WEIGHTS_FILE, {})
    normalized_weights = {}

    for cat, weight in raw_weights.items():
        if not isinstance(cat, str):
            continue
        normalized_weights[cat] = min(max(_as_float(weight, 5.0), 1.0), 10.0)
...
def load_goals():
    ...
    for name, goal_data in raw_goals.items():
        if not isinstance(name, str) or not isinstance(goal_data, dict):
            continue

        normalized_goals[name] = {
            "target": max(_as_float(goal_data.get("target"), 0.0), 0.0),
            "saved": max(_as_float(goal_data.get("saved"), 0.0), 0.0),
        }
...
def load_wallet():
    ...
    """
    wallet_data = _load_json_file(WALLET_FILE, DEFAULT_WALLET)
    wallet_data.setdefault("budget", 0.0)
    wallet_data.setdefault("expense_items", [])
    wallet_data["budget"] = max(_as_float(wallet_data.get("budget"), 0.0), 0.0)

    if not isinstance(wallet_data["expense_items"], list):
        wallet_data["expense_items"] = []

    normalized_expenses = []
    for item in wallet_data["expense_items"]:
    ...
    wallet_data["expense_items"] = normalized_expenses
    return wallet_data
...
def load_categories():
    ...
    """
    raw_categories = _load_json_file(CATEGORIES_FILE, DEFAULT_CATEGORIES)
    normalized_categories = {}

    for cat, coeff in raw_categories.items():
        if not isinstance(cat, str):
            continue

        coeff_value = _as_float(coeff, 0.0)
        if coeff_value > 0:
            normalized_categories[cat] = coeff_value
...
def ensure_goal_category_exists():
    """Гарантирует существование категории накоплений для переводов по целям."""
    categories = load_categories()
    weights = load_weights()
    updated = False
    ...
    if GOAL_EXPENSE_CATEGORY not in weights:
        weights[GOAL_EXPENSE_CATEGORY] = 5
        updated = True
...
def record_goal_transfer(goal_name, amount):
    """Записывает перевод по цели как движение в категории накоплений."""
    del goal_name  # Имя цели пока не хранится отдельно в расходах.

    transfer_amount = round(_as_float(amount, 0.0), 2)
    if transfer_amount == 0:
        return

    ensure_goal_category_exists()

    wallet_data = load_wallet()
    expenses = wallet_data.get("expense_items", [])
    expenses.append({
        "category": GOAL_EXPENSE_CATEGORY,
    ...
    if has_streamlit_context() and "expense_items" in st.session_state:
...
# ================================
# ВООБЩЕ ВАМ ТРОГАТЬ НЕ НАДО, ТУТ НИЧЕГО СИЛЬНО НЕТУ ДЛЯ ВАС
    ...
    if budget > 0:
        if expenses_count > 0:
            ...
            st.markdown(
                ...
                f"Текущий бюджет: {budget:.2f} ₽<br>"
                f"Добавлено расходов: {expenses_count} на сумму {total_spent:.2f} ₽<br>"
...
def page_expenses():
    ...
    budget = st.number_input(
        ...
        step=100.0,
        format="%.2f",
        value=current_budget if current_budget > 0 else 0.01,
        key="budget_input",
        label_visibility="collapsed",
        width="stretch",
    )
    if st.button("Сохранить изменения", width="stretch", type="primary"):
    ...
    with st.expander("Информация о распределении бюджета"):
        ...
        render_table(category_data, width="stretch")
    ...
    add_button = st.button("Добавить расход", width="stretch", type="primary")
    ...
    if st.session_state.expense_items:
        ...
        render_table(expense_data, width="stretch", height=400)
        ...
        with col1:
            if st.button("Очистить все расходы", width="stretch", type="primary"):
...
def page_goals():
    ...
    with st.expander("Создать новую цель"):
        ...
        if st.button("Создать цель", width="stretch", type="primary"):
            if not new_name:
    ...
    if goals:
        with st.expander("Перевод между бюджетом и целью"):
            ...
            if st.button("Выполнить перевод", width="stretch", type="primary"):
                if amount <= 0:
                    st.warning("Сумма должна быть положительной.")
                    ...
                    if direction == "В цель":
                        ...
                        st.warning(f"Сумма учтена как расход в категории '{GOAL_EXPENSE_CATEGORY}'")
                    else:
                        st.warning(f"Возвращено {amount:.2f} ₽ из цели '{selected}' в бюджет")
                        st.warning(f"Сумма вычтена из расходов по категории '{GOAL_EXPENSE_CATEGORY}'")
                    st.rerun()
        ...
        with st.expander("Удалить цель"):
            ...
            if st.button("Удалить цель", width="stretch", type="primary"):
                if saved_amount > 0:
...
def page_categories():
    ...
    if categories and len(categories) > 0:
        ...
        if current_budget > 0:
            st.warning(f"Текущий бюджет: {current_budget:.2f} ₽ — рекомендуемые суммы пересчитаны исходя из него.")
        else:
            st.warning(
        ...
        if current_budget > 0 and categories:
            ...
            df_pie = pd.DataFrame({
                ...
                'Доля бюджета (%)': [coeff * 100 for coeff in categories.values()],
                'Сумма (₽)': [current_budget * coeff for coeff in categories.values()]
            }).sort_values('Сумма (₽)', ascending=False)
            ...
            fig = px.pie(
                ...
                values='Сумма (₽)',
                names='Категория',
                title='Рекомендуемое распределение бюджета',
        ...
        if abs(total - 1.0) > 0.001:
            ...
            if st.button("Нормализовать коэффициенты", use_container_width=True):
                recalc_coefficients_from_weights(weights)
                st.warning("Коэффициенты нормализованы. Важность категорий сохранена.")
                st.rerun()
    else:
    ...
    with st.expander("Добавить новую категорию", expanded=(not categories or len(categories) == 0)):
        ...
        with col1:
            if st.button("Создать категорию", use_container_width=True, type="primary", key="create_category_btn"):
                if not new_category_name:
                    st.warning("Название категории не может быть пустым!")
                elif categories and new_category_name in categories:
                    st.warning(f"Категория '{new_category_name}' уже существует!")
                else:
                    if not weights:
                        weights = {}
        ...
        with col2:
            if weights:
                ...
                st.warning(f"При добавлении категории с важностью {importance}\n"
                           f"её доля бюджета составит примерно {preview_coeff * 100:.1f}%\n"
                           f"→ рекомендуемая сумма: {preview_limit:.2f} ₽")
            else:
                st.warning("Это будет первая категория. Её доля бюджета составит 100%.")
    ...
    if categories and len(categories) > 0:
        ...
        with st.expander("Изменить важность категории"):
            ...
            if st.button("Сохранить изменение", use_container_width=True, key="save_importance_btn"):
                if new_importance == current_importance:
                    st.warning("Значение не изменилось.")
                else:
            ...
            with col1:
                if st.button("Удалить категорию", use_container_width=True, key="delete_category_btn"):
                    if len(categories) <= 1:
                        ...
                        save_weights(weights)
                        recalc_coefficients_from_weights(weights)
                        st.warning(f"Категория '{deleted_name}' полностью удалена!")
                        if removed_count > 0:
                            st.warning(f"Удалено расходов: {removed_count} на сумму {removed_amount:.2f} ₽")
                        st.rerun()

            with col2:
                st.warning(f"Количество категорий после удаления: {len(categories) - 1}")
        ...
        with st.expander("Сбросить важность всех категорий"):
            if st.button("Сбросить важность всех категорий к значению 5", use_container_width=True,
                ...
                save_weights(weights)
                recalc_coefficients_from_weights(weights)
                st.warning("Важность всех категорий сброшена к 5! Все категории имеют равные доли бюджета.")
                st.rerun()
...
# ГЛАВНОЕ МЕНЮ
# ================================
# ВОТ ЭТО ВНИЗУ ЛУЧШЕ НЕ ТРОГАТЬ ВООБЩЕ
